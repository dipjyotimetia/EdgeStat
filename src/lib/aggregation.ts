...
export async function getOverviewStats(
  ...
  const [current, sessionStats, durationStat, previous] = await Promise.all([
    ...
    db
      ...
      .bind(siteId, fromTs, toTs)
      .first<{ avg_session_duration: number }>(),
    ...
    db
      ...
      .bind(siteId, prevFromTs, prevToTs)
      .first<{ pageviews: number; visitors: number }>(),
  ]);
...
export async function getCustomEvents(
  ...
  events: { event_name: string; count: number; properties: Record<string, string | number> }[];
}> {
  const fromTs = dateToTimestamp(from);
  const toTs = endOfDay(to);
  ...
  if (name) {
    const { results } = await db
      ...
      .bind(siteId, name, fromTs, toTs, limit)
      .all<{ event_name: string; event_props: string | null; count: number }>();

    return {
      events: results.map((r) => ({
        event_name: r.event_name,
        count: r.count,
        properties: r.event_props
          ? (JSON.parse(r.event_props) as Record<string, string | number>)
          : {},
      })),
    };
  }
...
export async function getFunnelConversion(
  db: D1Database,
  siteId: string,
  steps: { type: 'url' | 'event'; value: string }[],
  from: string,
  to: string
): Promise<{ step: number; name: string; visitors: number; conversion: number }[]> {
  const fromTs = dateToTimestamp(from);
  const toTs = endOfDay(to);
  const results: { step: number; name: string; visitors: number; conversion: number }[] = [];
