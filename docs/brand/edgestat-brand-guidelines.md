...
```css
font-family:
  ui-monospace, 'Cascadia Code', 'Fira Code', 'JetBrains Mono', 'Source Code Pro', monospace;
```
...
_Edgestat brand guidelines — maintained by the core team._
_Questions or suggestions → open an issue in `dipjyotimetia/edgestat`._
